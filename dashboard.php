...
<section class="dashboard">
   ...
   <div class="box-container">
      ...
      <div class="box">
         <h3>Filter Search</h3>
         <p>Search Your Dream Residence</p>
         <a href="search.php" class="btn">Search Now</a>
      </div>

      <div class="box">
      <?php
        $count_residences = $conn->prepare("SELECT * FROM `residence` WHERE user_id = ?");
        $count_residences->execute([$user_id]);
        $total_residences = $count_residences->rowCount();
      ?>
      <h3><?= $total_residences; ?></h3>
      <p>Residences Listed</p>
      ...
      <p>Requests Sent</p>
      <a href="saved.php" class="btn">View Saved Residences</a>
      </div>

      <div class="box">
      <?php
        $count_saved_residences = $conn->prepare("SELECT * FROM `saved` WHERE user_id = ?");
        $count_saved_residences->execute([$user_id]);
        $total_saved_residences = $count_saved_residences->rowCount();
      ?>
      <h3><?= $total_saved_residences; ?></h3>
      <p>Residences Saved</p>
      <a href="saved.php" class="btn">View Saved Residences</a>
      </div>
