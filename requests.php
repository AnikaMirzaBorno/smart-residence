...
<section class="requests">
   ...
   <?php
      ...
      if($select_requests->rowCount() > 0){
        ...
        $select_residence = $conn->prepare("SELECT * FROM `residence` WHERE id = ?");
        $select_residence->execute([$fetch_request['residence_id']]);
        $fetch_residence = $select_residence->fetch(PDO::FETCH_ASSOC);
   ?>
   <div class="box">
      ...
      <p>enquiry for : <span><?= $fetch_residence['residence_name']; ?></span></p>
